...
def assignPoints(tbl, ctrs):
    ...
    ptsAsgn = []
    for point in tbl:
        distances = [euclideanDist(point, ctr) for ctr in ctrs]
...
def recalculateCtrs(tbl, ctrs, ptsAsgn):
    ...
    for i, ctr in enumerate(ctrs):
        assigned_points = [tbl[j] for j in range(len(ptsAsgn)) if ptsAsgn[j] == i]
        if assigned_points:
            new_ctr = [sum(point) / len(assigned_points) for point in zip(*assigned_points)]
        else:
            new_ctr = ctr
        newCtrs.append(new_ctr)
    return newCtrs
