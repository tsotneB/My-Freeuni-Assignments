...
def seqalignDP(seq1, seq2, subst_matrix, gap_pen):
    ...
    for i in range(1, len(seq1) + 1):
        for j in range(1, len(seq2) + 1):
            fb = base_idx[seq1[i-1]]
            sb = base_idx[seq2[j-1]]
            both_extended = F[i-1][j-1]+subst_matrix[fb][sb]
            gap_seq2 = F[i-1][j]-gap_pen
            gap_seq1 = F[i][j-1]-gap_pen
            scores = [(both_extended, PTR_BASE), (gap_seq2, PTR_GAP2), (gap_seq1, PTR_GAP1)]
            scores_sorted = sorted(scores, key=lambda x: x[0])
            result = scores_sorted[2]
            F[i][j]= result[0]
            TB[i][j]= result[1]
