...
from util import plothist, argmax
...
def viterbi(X):
    ...
    for i in xrange(0,L):
        ...
        for k in xrange(N):
            ...
            emmision = log(em[k][X[i]])
            transitions = [Vprev[j] + log(tr[j][k]) for j in range(N)]
            V[i][k] = emmision + max(transitions)
            TB[i][k] = argmax(transitions)
