    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        bestAction = None
        bestDistance = 93342349304820348
        for dist in livingGhostPositionDistributions:
            mostProbPos = None
            mostProb = 0
            for ghostPos in dist.keys():
                if dist[ghostPos] > mostProb:
                    mostProbPos = ghostPos
                    mostProb = dist[ghostPos]
            for action in legal:
                successorPosition = Actions.getSuccessor(pacmanPosition, action)
                currDist = self.distancer.getDistance(successorPosition,mostProbPos)
